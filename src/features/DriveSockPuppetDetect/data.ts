...
import type { TableExtractionData, DriveSockPuppetData, DriveSockPuppetItem } from './types'
import { DATA_STORAGE_KEY, DEFAULT_SOCK_PUPPET_DATA, LAST_UPDATE_KEY, AUTO_UPDATE_INTERVAL } from './config'

/**
 * 从表格行数组解析马甲数据（通用解析逻辑）
 */
const parseRowsToSockPuppetItems = (rows: Array<{ content: string }[]>): DriveSockPuppetData => {
  const sockPuppetItems: DriveSockPuppetItem[] = []
  let currentForumId = ''
  let currentNote = ''
  ...
  rows.forEach(cells => {
    if (cells.length >= 7) {
      ...
      if (serialNumber !== '-' && forumId && forumId !== '-') {
        ...
        // 查找是否已存在该用户的记录
        let existingItem = sockPuppetItems.find(item => item.forumId === currentForumId)
        ...
        if (!existingItem) {
          ...
          sockPuppetItems.push(existingItem)
        }
      }

      // 如果有当前用户且网盘厂商有值，添加网盘记录
      if (currentForumId && provider && provider.trim() !== '') {
        const existingItem = sockPuppetItems.find(item => item.forumId === currentForumId)
        if (existingItem) {
          // 检查是否已存在相同的网盘记录
  ...
  return sockPuppetItems
}

/**
 * 从 DOM 直接解析表格数据（备用方案，解决异步时序问题）
 */
const parseTableFromDOM = (): DriveSockPuppetData | null => {
  // 查找目标表格
  ...
  // console.log('[检测网盘马甲] 从 DOM 直接解析表格数据')
  ...
  return parseRowsToSockPuppetItems(rows)
}

/**
 * 从表格数据提取器加载马甲数据
 */
export const loadSockPuppetDataFromTableExtractor = async (): Promise<DriveSockPuppetData> => {
  try {
    // console.log('[检测网盘马甲] 尝试从表格数据提取器加载数据')
    ...
    if (extractionData) {
      // console.log('[检测网盘马甲] 找到表格数据提取器数据，结构:', Object.keys(extractionData))
      ...
      if (extractionDataTyped.targetTable && extractionDataTyped.targetTable.rows) {
        // console.log('[检测网盘马甲] 目标表格行数:', extractionDataTyped.targetTable.rows.length)

        // 从提取器数据解析
        const rows = extractionDataTyped.targetTable.rows.map(row => row.cells)
        const parsedData = parseRowsToSockPuppetItems(rows)

        if (parsedData.length > 0) {
          // console.log(`[检测网盘马甲] 从表格提取器解析出 ${parsedData.length} 个用户`)
          return parsedData
        }
      }
    }
    ...
    console.log('[检测网盘马甲] 表格提取器数据不可用，尝试从 DOM 直接解析')
    const domParsedData = parseTableFromDOM()

    if (domParsedData && domParsedData.length > 0) {
      // console.log(`[检测网盘马甲] 从 DOM 解析出 ${domParsedData.length} 个用户`)
      return domParsedData
    }

    // 如果都失败，使用默认数据
    // console.log('[检测网盘马甲] 无法解析表格数据，使用默认数据')
    return DEFAULT_SOCK_PUPPET_DATA
  } catch (error) {
    // console.error('[检测网盘马甲] 从表格数据提取器加载数据失败:', error)
    return DEFAULT_SOCK_PUPPET_DATA
  }
}
 ...
 * 加载马甲数据
 */
export const loadSockPuppetData = async (): Promise<DriveSockPuppetData> => {
  // 检查当前页面是否是目标页面（马甲数据源页面）
  const isTargetPage = window.location.href.includes('thread-2094795-1-1.html')

  // 如果是目标页面，优先从表格数据提取器重新加载最新数据
  if (isTargetPage) {
    // console.log('[检测网盘马甲] 当前是目标页面，从表格数据提取器重新加载数据')
    const extractedData = await loadSockPuppetDataFromTableExtractor()
    if (extractedData.length > 0) {
      await saveSockPuppetData(extractedData)
  ...
  // 从本地存储加载数据
  const storedData = await storageHelper.loadArray<DriveSockPuppetItem>(DATA_STORAGE_KEY, [])

  // 如果本地存储有数据，使用存储的数据
  if (storedData.length > 0) {
    // console.log(`[检测网盘马甲] 从本地存储加载 ${storedData.length} 条数据`)
    return storedData
  }

  // 如果本地存储没有数据，从表格数据提取器加载作为初始数据
  // console.log('[检测网盘马甲] 本地存储无数据，从表格数据提取器加载初始数据')
  const extractedData = await loadSockPuppetDataFromTableExtractor()
 ...
 * 保存马甲数据
 */
export const saveSockPuppetData = async (data: DriveSockPuppetData): Promise<void> => {
