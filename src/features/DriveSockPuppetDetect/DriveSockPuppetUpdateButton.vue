<template>
  <div class="drive-sock-puppet-update-container">
    <button class="btn-update" @click="updateSockPuppetData" :disabled="isUpdating">
      {{ isUpdating ? '更新中...' : '更新网盘马甲' }}
    </button>
  </div>
</template>
...
// 更新检测网盘马甲数据
const updateSockPuppetData = async (): Promise<void> => {
  ...
  try {
    ...
    // 显示成功通知
    success('检测网盘马甲数据已更新', { title: '数据更新' })

    // 触发更新事件
    emit('update')
  } catch (err) {
    console.error('更新检测网盘马甲数据失败:', err)
...
<style scoped>
.drive-sock-puppet-update-container {
  display: flex;
