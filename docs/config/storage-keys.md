...
// 获取多个键值
const results = await browser.storage.local.get(['avatarQueryEnabled', 'quickReplyEnabled', 'contentFilterEnabled'])
...
```javascript
// 清除头像查询相关存储
chrome.storage.local.remove(['avatarQueryEnabled', 'avatarQueryHistory'])
```
